...
const seedLibrary: Q[] = [
  { id: "q1", section: "Screen-in", text: "Which of the following best describes your role in household grocery shopping?", type: TYPE_SINGLE, options: ["I am the primary shopper", "I share responsibilities", "I am not involved"], instructions: "Confirm respondent is primary or shares responsibilities. If not involved, terminate.", tags: ["general", "shopping", "household"]},
  { id: "q2", section: "Category Usage", text: "How often do you purchase beverages for your household?", type: TYPE_SINGLE, options: ["Weekly or more", "Every 2–3 weeks", "Monthly", "Less often / never"], instructions: "Prioritize WEEKLY and EVERY 2–3 WEEKS for frequent users unless otherwise specified.", tags: ["beverages", "frequency", "usage"]},
  { id: "q3", section: "Category Usage", text: "Which of the following snack types do you purchase at least once a month?", type: TYPE_MULTI, options: ["Chips & crisps", "Cookies & biscuits", "Crackers", "Nuts & trail mix", "Candy", "None of the above"], instructions: "Probe for typical brands. IF 'NONE', VERIFY RELEVANCE BEFORE CONTINUING.", tags: ["snacks", "purchase", "assortment"]},
];
...
export default function ScreenerBuilderApp() {
  ...
  function removeOption(optIndex: number) {
    setBuilderQs((p) =>
      p.map((q) =>
        q.id === selectedId ? { ...q, options: removeOptionFromList(q.options || [], optIndex) } : q
      )
    );
  }
  ...
  async function exportToWord() {
    ...
    try {
      headerChildren.push(
        new Paragraph({
          children: [
            new ImageRun({ data: dataUrlToUint8Array(BRAND_LOGO_DATA_URL), transformation: { width: 200, height: 60 } }),
          ],
        })
      );
    } catch (e) {
    ...
    try {
      const doc = new Document({
        styles: {
          ...
          default: {
            heading1: { run: { size: 32, bold: true, font: "Calibri" } },
            heading2: { run: { size: H2_SIZE, bold: true, color: BRAND_MAGENTA, font: "Calibri" } },
            paragraph: { run: { size: BODY_FONT_SIZE, font: "Calibri" } },
          },
        },
        sections: [
          {
                  ...
                  if (q.type !== TYPE_OPEN && (q.options || []).length) {
                    arr.push(
                      ...(q.options || []).map((opt: string, i: number) =>
                        new Paragraph({ children: [new TextRun({ text: `  ${String.fromCharCode(97 + i)}. ${opt}` })] })
                      )
                    );
                  }
                  if (q.instructions) {
                    arr.push(
                      new Paragraph({
                        children: [
                          new TextRun({ text: (q.instructions || "").toUpperCase(), color: BRAND_MAGENTA, italics: true }),
                        ],
                      })
                    );
                  }
        ...
        {lastDownload && (
          <div className="mx-auto max-w-7xl px-4 pb-3">
            <div className="text-xs text-slate-600 bg-slate-100 border border-slate-200 rounded p-2">
              If your download didn’t start, <a className="underline" href={lastDownload.url} download={lastDownload.name} target="_blank" rel="noopener noreferrer">click here</a> to save the {lastDownload.kind.toUpperCase()} file, or <button className="underline" onClick={() => window.open(lastDownload.url, '_blank')}>open in a new tab</button>.
            </div>
          </div>
        )}
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 grid grid-cols-12 gap-4">
        <section className="col-span-3 space-y-4">
          <div className="bg-white rounded-2xl shadow p-4 space-y-3">
            ...
            <button className="px-3 py-2 rounded bg-slate-900 text-white w-full" onClick={loadBehaviorallyStandard}>Load Behaviorally Standard Order</button>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-4 space-y-3">
            ...
            <div className="text-xs text-slate-500">{mode === MODE_ONLINE ? "Online: device checks will be included" : "In-person: device checks will be omitted in export"}</div>
            <div className="space-y-2 max-h-[320px] overflow-auto pr-1">
              {suggested.length === 0 && (
                <div className="text-sm text-slate-500">No suggestions yet. Add specs or change category.</div>
              )}
              {suggested.map((q) => (
                <div key={q.id} className="border rounded p-2">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <button className="text-xs px-2 py-1 rounded bg-slate-900 text-white" onClick={() => addToBuild(q)}>Add</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-2xl shadow p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={addCustom} className="px-3 py-2 rounded bg-slate-900 text-white">Add Custom</button>
              </div>
            </div>

            {builderQs.length === 0 ? (
              <div className="text-sm text-slate-500 mt-4">Use <strong>Load Behaviorally Standard Order</strong>, then tweak with <strong>Suggested</strong> or <strong>Library</strong>.</div>
            ) : (
              <ul className="mt-4 space-y-2">
                {builderQs.map((q, i) => (
                  <li key={q.id} className={`border rounded p-3 ${selectedId === q.id ? "ring-2 ring-blue-400" : ""}`}>
                    <div className="flex items-start gap-3 cursor-pointer" onClick={() => setSelectedId(q.id)}>
                      ...
                      <div className="flex-1">
                        ...
                        {q.instructions && (
                          <div className="text-sm mt-2"><span className="text-[#C000A3] italic font-medium">{(q.instructions || "").toUpperCase()}</span></div>
                        )}
                        {q.tags?.length ? (
                          <div className="mt-2 flex flex-wrap gap-1">
                            {q.tags.map((t: string, idx: number) => (
                              <span key={idx} className="px-2 py-0.5 text-xs rounded-full bg-slate-100 border">{t}</span>
                            ))}
                          </div>
                        ) : null}
                      </div>
                      <div className="flex flex-col gap-1">
                        <button className="text-xs px-2 py-1 rounded bg-white border" onClick={(e) => { e.stopPropagation(); setSelectedId(q.id); }}>Edit</button>
                        <button className="text-xs px-2 py-1 rounded bg-white border" onClick={() => move(i, -1)}>▲</button>
                        <button className="text-xs px-2 py-1 rounded bg-white border" onClick={() => move(i, +1)}>▼</button>
                        <button className="text-xs px-2 py-1 rounded bg-red-50 text-red-700 border border-red-200" onClick={() => removeFromBuild(q.id)}>Remove</button>
                      </div>
                    </div>
                  </li>
                ))}
        ...
        <aside className="col-span-3">
          <div className="bg-white rounded-2xl shadow p-4" ref={inspectorRef}>
            ...
            {!selectedQ ? (
              <div className="text-sm text-slate-500 mt-2">Select a question from the builder to edit.</div>
            ) : (
              <div className="space-y-2 mt-2">
                <label className="block text-xs uppercase text-slate-500">Section</label>
                <input className="w-full border rounded px-2 py-1" value={selectedQ.section || ""} onChange={(e) => updateSelected("section", e.target.value)} />

                <label className="block text-xs uppercase text-slate-500">Question Text</label>
                <textarea className="w-full border rounded px-2 py-1 h-24" value={selectedQ.text || ""} onChange={(e) => updateSelected("text", e.target.value)} ref={questionTextRef} />

                <label className="block text-xs uppercase text-slate-500">Type</label>
                <select className="w-full border rounded px-2 py-1" value={selectedQ.type || TYPE_SINGLE} onChange={(e) => updateSelected("type", e.target.value as QType)}>
                ...
                {selectedQ.type !== TYPE_OPEN && (
                  <div className="border rounded p-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button className="text-xs px-2 py-1 rounded bg-slate-900 text-white" onClick={addOption}>Add</button>
                    </div>
                    <div className="space-y-1 mt-2">
                      {(selectedQ.options || []).map((o: string, i: number) => (
                        <div key={i} className="flex items-center gap-2">
                          <input className="flex-1 border rounded px-2 py-1" value={o} onChange={(e) => updateOption(i, e.target.value)} />
                          <button className="text-xs px-2 py-1 rounded bg-white border" onClick={() => removeOption(i)}>Remove</button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <label className="block text-xs uppercase text-slate-500">Recruiter Instructions</label>
                <textarea className="w-full border rounded px-2 py-1 h-24" value={selectedQ.instructions || ""} onChange={(e) => updateSelected("instructions", e.target.value)} />

                <label className="block text-xs uppercase text-slate-500">Tags (; separated)</label>
                <input className="w-full border rounded px-2 py-1" value={(selectedQ.tags || []).join("; ")} onChange={(e) => updateSelected("tags", e.target.value.split(/;\s*/).filter(Boolean))} />
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow p-4 mt-4">
            ...
            <ul className="list-disc ml-5 text-sm space-y-1 text-slate-600">
              <li>Click <strong>Load Behaviorally Standard Order</strong> to start from your house style.</li>
              <li>Mode toggles device checks: Online includes them, In-person omits them in export.</li>
