...
const seedLibrary: Q[] = [
  {
    ...
    instructions: "Confirm respondent is primary or shares responsibilities. If not involved, terminate.",
  ...
  },
  {
    ...
    instructions: "Prioritize WEEKLY and EVERY 2–3 WEEKS for frequent users unless otherwise specified.",
  ...
  },
  {
    ...
    type: TYPE_MULTI,
    options: ["Chips & crisps", "Cookies & biscuits", "Crackers", "Nuts & trail mix", "Candy", "None of the above"],
    instructions: "Probe for typical brands. IF 'NONE', VERIFY RELEVANCE BEFORE CONTINUING.",
...
export default function ScreenerBuilderApp() {
  ...
  function updateDraftOption(idx: number, value: string) {
    if (!draft) return;
    setDraft({ ...draft, options: updateOptionInList(draft.options || [], idx, value) });
  }
  function addDraftOption() {
    if (!draft) return;
    setDraft({ ...draft, options: addOptionToList(draft.options || []) });
  }
  function removeDraftOption(idx: number) {
    if (!draft) return;
    setDraft({ ...draft, options: removeOptionFromList(draft.options || [], idx) });
  }
  ...
  async function exportToWord() {
    ...
    try {
      headerChildren.push(
        new Paragraph({
          children: [new ImageRun({ data: dataUrlToUint8Array(BRAND_LOGO_DATA_URL), transformation: { width: 200, height: 60 } })],
        })
      );
    } catch {}
            ...
            children: [
              ...
              ...builderQs
                ...
                .flatMap((q, idx) => {
                  ...
                  if (q.instructions) {
                    parts.push(new Paragraph({ children: [new TextRun({ text: (q.instructions || "").toUpperCase(), color: BRAND_MAGENTA, italics: true })] }));
                  }
  ...
  function exportAsHtmlDoc() {
    ...
    const html =
      ...
      builderQs
        ...
        .map((q, i) => {
          const opts =
            q.type !== TYPE_OPEN && (q.options || []).length
      ...
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            <input className="px-2 py-1 border rounded w-[260px]" value={studyTitle} onChange={(e) => setStudyTitle(e.target.value)} placeholder="AG### | Project Name" />
            <button onClick={exportToWord} className="px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-700 shadow" title="Export to Word">
              Export to Word
            </button>
          </div>
        </div>
        {lastDownload && (
          <div className="mx-auto max-w-7xl px-4 pb-3">
            <div className="text-xs text-slate-600 bg-slate-100 border border-slate-200 rounded p-2">
              ...
              <button className="underline" onClick={() => window.open(lastDownload.url, "_blank")}>open in a new tab</button>.
            </div>
          </div>
        )}
      </header>
        ...
        <section className="col-span-3 space-y-4">
          <div className="bg-white rounded-2xl shadow p-4 space-y-3">
            ...
            <select className="w-full border rounded px-2 py-1" value={category} onChange={(e) => setCategory(e.target.value)}>
              <option>Beverages</option><option>Snacks</option><option>Personal Care</option><option>Household</option><option>Other</option>
            </select>
            ...
            {builderQs.length === 0 ? (
              ...
              <ul className="mt-4 space-y-2">
                {builderQs.map((q, i) => {
                  ...
                  return (
                    <li key={q.id} className={`border rounded p-3 ${isEditing ? "ring-2 ring-blue-400" : ""}`}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          {!isEditing ? (
                            <>
                              ...
                              {q.instructions && (
                                <div className="text-sm mt-2">
                                  <span className="text-[#C000A3] italic font-medium">{(q.instructions || "").toUpperCase()}</span>
                                </div>
                              )}
                              {q.tags?.length ? (
                            ...
                            <>
                              ...
                              <textarea className="w-full border rounded px-2 py-1 h-24" value={draft?.text || ""} onChange={(e) => updateDraft("text", e.target.value)} ref={questionTextRef} />

                              <label className="block text-xs uppercase text-slate-500 mt-2">Type</label>
                              <select className="w-full border rounded px-2 py-1" value={draft?.type || TYPE_SINGLE} onChange={(e) => updateDraft("type", e.target.value as QType)}>
                              ...
                              {draft?.type !== TYPE_OPEN && (
                                <div className="border rounded p-2 mt-2">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <button className="text-xs px-2 py-1 rounded bg-slate-900 text-white" onClick={addDraftOption}>Add</button>
                                  </div>
                                  <div className="space-y-1 mt-2">
                                    {(draft?.options || []).map((o, idx) => (
                                      <div key={idx} className="flex items-center gap-2">
                                        ...
                                        <button className="text-xs px-2 py-1 rounded bg-white border" onClick={() => removeDraftOption(idx)}>Remove</button>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                              ...
                              <input className="w-full border rounded px-2 py-1" value={(draft?.tags || []).join("; ")} onChange={(e) => updateDraft("tags", e.target.value.split(/;\s*/).filter(Boolean))} />
                            </>
                          )}
                        </div>
                        ...
                        <div className="flex flex-col gap-1 items-end">
                          {!isEditing ? (
                            <>
                              <button className="text-xs px-2 py-1 rounded bg-white border" onClick={() => startEdit(q)}>Edit</button>
                              <div className="flex items-center gap-1">
                                <span className="text-[11px] text-slate-500">Move to</span>
                                <select className="text-xs border rounded px-1 py-0.5" value={i + 1} onChange={(e) => setBuilderQs((prev) => moveToIndex(prev, i, Number(e.target.value) - 1))}>
                                  {builderQs.map((_, n) => (<option key={n} value={n + 1}>{n + 1}</option>))}
                                </select>
                              </div>
                              <button className="text-xs px-2 py-1 rounded bg-white border" onClick={() => setBuilderQs((prev) => moveToIndex(prev, i, i - 1))} disabled={i === 0}>▲</button>
                              <button className="text-xs px-2 py-1 rounded bg-white border" onClick={() => setBuilderQs((prev) => moveToIndex(prev, i, i + 1))} disabled={i === builderQs.length - 1}>▼</button>
                            ...
                            <>
                              <button className="text-xs px-2 py-1 rounded bg-emerald-600 text-white" onClick={saveEdit}>Save</button>
                              <button className="text-xs px-2 py-1 rounded bg-white border" onClick={cancelEdit}>Cancel</button>
                            </>
                          )}
                        </div>
                      </div>
